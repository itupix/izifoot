...
exports.ALLOWED_TEAM_CATEGORIES = void 0;
exports.normalizeTeamCategory = normalizeTeamCategory;
const MIN_U_CATEGORY = 6;
const MAX_U_CATEGORY = 20;
const NORMALIZED_NON_U_CATEGORIES = ['Seniors', 'Vétérans'];
exports.ALLOWED_TEAM_CATEGORIES = [
...
function normalizeCategoryToken(token) {
    ...
    if (uMatch) {
        ...
        if (!Number.isInteger(age) || age < MIN_U_CATEGORY || age > MAX_U_CATEGORY)
            return null;
        return `U${age}`;
    }
    const folded = stripDiacritics(trimmed).toLowerCase();
    if (folded === 'senior' || folded === 'seniors')
        return 'Seniors';
    if (folded === 'veteran' || folded === 'veterans')
        return 'Vétérans';
    return null;
}
function parseUAge(category) {
...
function normalizeTeamCategory(rawCategory) {
