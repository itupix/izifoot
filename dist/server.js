...
const MOBILE_AUTH_CODE_TTL_SECONDS = readPositiveIntEnv(process.env.MOBILE_AUTH_CODE_TTL_SECONDS, mobile_auth_1.DEFAULT_MOBILE_AUTH_CODE_TTL_SECONDS);
const MOBILE_AUTH_STATE_TTL_SECONDS = readPositiveIntEnv(process.env.MOBILE_AUTH_STATE_TTL_SECONDS, mobile_auth_1.DEFAULT_MOBILE_AUTH_STATE_TTL_SECONDS);
...
function mobileAuthCookieOpts(maxAgeMs) {
    const isProd = process.env.NODE_ENV === 'production';
    const sameSite = isProd ? 'none' : 'lax';
    return {
        httpOnly: true,
...
function parseOptionalSeasonId(raw) {
    if (typeof raw !== 'string')
        return null;
    const value = raw.trim();
    return value.length > 0 ? value : null;
}
function toClubResponse(club, currentSeason = null) {
    if (!club)
        return null;
    const { seasonStartMonth, seasonStartDay, seasonEndMonth, seasonEndDay, seasonTimezone, ...rest } = club;
    return {
        ...rest,
        seasonConfig: (0, season_1.normalizeClubSeasonConfig)({
            seasonStartMonth,
            seasonStartDay,
            seasonEndMonth,
            seasonEndDay,
        ...
        currentSeason: currentSeason ? (0, season_1.toSeasonResponse)(currentSeason) : null,
    };
}
function toEntityWithSeason(entity) {
    if (!entity)
        return entity;
    const season = entity.season ? (0, season_1.toSeasonResponse)(entity.season) : null;
    return {
        ...entity,
...
async function resolveSeasonForClubId(db, clubId, dateInput) {
    if (!clubId)
        return null;
    ...
    if (!club)
        return null;
    return (0, season_1.resolveOrCreateSeasonForClubDate)(db, club, dateInput);
}
async function clubHasMaterializedSeasonData(db, clubId) {
    ...
    return seasons.some((season) => (season._count.trainings > 0
...
async function backfillLegacySeasonData() {
    try {
        ...
        for (const training of trainings) {
            ...
            if (!club)
                continue;
            const season = await (0, season_1.resolveOrCreateSeasonForClubDate)(prisma, club, training.date);
            if (!season)
                continue;
        ...
        for (const plateau of plateaus) {
            ...
            if (!club)
                continue;
            const season = await (0, season_1.resolveOrCreateSeasonForClubDate)(prisma, club, plateau.date);
            if (!season)
                continue;
        ...
        for (const match of matches) {
            ...
            const season = plateau?.seasonId
                ? { id: plateau.seasonId }
                : await (0, season_1.resolveOrCreateSeasonForClubDate)(prisma, club, nextDate);
...
function toMobileAuthExchangeUser(user) {
    return {
        id: user.id,
        email: user.email,
...
function resolvePlayerRosterStatusFilter(raw) {
    if (raw === undefined || raw === null || raw === '')
        return 'active';
    if (Array.isArray(raw))
        return resolvePlayerRosterStatusFilter(raw[0]);
    if (typeof raw !== 'string')
        return null;
...
function playerRosterStatusWhere(status) {
    switch (status) {
        case 'active':
...
function isPlayerActiveRecord(player) {
...
function collectCoachTeamIds(rows) {
    const ids = new Set();
    for (const row of rows) {
        for (const teamId of (0, coach_management_1.normalizeCoachManagedTeamIds)(row.teamId, row.managedTeamIds)) {
            ids.add(teamId);
        }
    }
    return Array.from(ids);
}
async function loadCoachTeamNameMap(clubId, rows) {
...
function toCoachSummaryFromUser(user, teamNameById = new Map()) {
    const managedTeamIds = (0, coach_management_1.normalizeCoachManagedTeamIds)(user.teamId, user.managedTeamIds);
    const activeTeamId = (0, coach_management_1.resolveCoachActiveTeamId)(user.teamId, managedTeamIds);
    return {
        id: user.id,
        firstName: user.firstName ?? null,
        lastName: user.lastName ?? null,
        ...
        managedTeamIds,
        managedTeams: (0, coach_management_1.mapCoachManagedTeams)(managedTeamIds, teamNameById),
        invitationStatus: 'ACCEPTED',
    };
}
function toCoachSummaryFromInvite(invite, teamNameById = new Map()) {
    const managedTeamIds = (0, coach_management_1.normalizeCoachManagedTeamIds)(invite.teamId, invite.managedTeamIds);
    const activeTeamId = (0, coach_management_1.resolveCoachActiveTeamId)(invite.teamId, managedTeamIds);
    return {
        id: invite.id,
...
app.get('/auth/mobile/start', async (req, res) => {
    const parsed = mobile_auth_1.mobileAuthStartQuerySchema.safeParse(req.query);
    if (!parsed.success) {
        const firstIssue = parsed.error.issues[0];
    ...
    const state = parsed.data.state || (0, mobile_auth_1.createMobileAuthState)();
    const stateCookie = (0, mobile_auth_1.signMobileAuthStateCookie)({
        platform,
        state,
        redirectUri,
    ...
    const callbackUrl = (0, mobile_auth_1.buildMobileAuthCallbackUrl)({
        apiBaseUrl: API_BASE_URL,
        state,
    });
    res.setHeader('Cache-Control', 'no-store');
    res.cookie(mobile_auth_1.MOBILE_AUTH_STATE_COOKIE_NAME, stateCookie, mobileAuthCookieOpts(MOBILE_AUTH_STATE_TTL_SECONDS * 1000));
    console.log('[auth.mobile.start]', { platform, hasSession: Boolean(auth?.id) });
    return res.redirect(302, auth
        ? callbackUrl
        : (0, mobile_auth_1.buildMobileAuthWebUrl)({
...
app.get('/auth/mobile/callback', async (req, res) => {
    const parsed = mobile_auth_1.mobileAuthCallbackQuerySchema.safeParse(req.query);
    if (!parsed.success) {
        res.clearCookie(mobile_auth_1.MOBILE_AUTH_STATE_COOKIE_NAME, mobileAuthClearCookieOpts());
        return res.status(400).json({ error: 'Invalid mobile auth state', code: 'MOBILE_AUTH_INVALID_STATE' });
    }
    const verifiedState = (0, mobile_auth_1.verifyMobileAuthStateCookie)({
        token: req.cookies?.[mobile_auth_1.MOBILE_AUTH_STATE_COOKIE_NAME],
        platform: 'ios',
    ...
    if (!auth) {
        ...
        return res.redirect(302, (0, mobile_auth_1.buildMobileAuthWebUrl)({
            appBaseUrl: APP_BASE_URL,
            platform: 'ios',
            state: parsed.data.state,
        }));
    }
    const code = (0, mobile_auth_1.createMobileAuthCode)();
    const codeHash = (0, mobile_auth_1.hashMobileAuthValue)(code, MOBILE_AUTH_SECRET);
    ...
    return res.redirect(302, (0, mobile_auth_1.buildMobileAuthAppCallbackUrl)({
...
app.post('/auth/mobile/exchange', async (req, res) => {
    const parsed = mobile_auth_1.mobileAuthExchangeBodySchema.safeParse(req.body);
    if (!parsed.success) {
        const firstIssue = parsed.error.issues[0];
        return res.status(400).json({ error: firstIssue?.message || 'Invalid mobile auth exchange payload' });
    }
    const exchange = await (0, mobile_auth_1.consumeMobileAuthExchange)({
...
app.put('/me/password', authMiddleware, async (req, res) => {
    const parsed = (0, account_password_1.parseMePasswordPutBody)(req.body);
    if (!parsed.success) {
        return res.status(400).json({
            error: 'Merci de renseigner votre mot de passe actuel et un nouveau mot de passe d au moins 6 caracteres.',
        });
    }
    const validated = (0, account_password_1.validatePasswordChangeInput)(parsed.data);
    if (!validated.ok) {
    ...
    if (!user)
        return res.status(404).json({ error: 'User not found' });
    const isCurrentPasswordValid = await bcryptjs_1.default.compare(parsed.data.currentPassword, user.passwordHash);
...
app.get('/clubs/me', authMiddleware, async (req, res) => {
    ...
    if (!club)
        return res.status(404).json({ error: 'Club not found' });
    const currentSeason = await (0, season_1.resolveOrCreateSeasonForClubDate)(prisma, club, new Date());
    res.json({
        ...toClubResponse(club, currentSeason),
        teams: (club.teams || []).map(withTeamFormatAliases),
    });
});
app.get('/clubs/me/seasons', authMiddleware, async (req, res) => {
    if (!req.auth?.clubId)
        return res.status(404).json({ error: 'Club not found' });
    const club = await prisma.club.findUnique({
        where: { id: req.auth.clubId },
    ...
    if (!club)
        return res.status(404).json({ error: 'Club not found' });
    await (0, season_1.resolveOrCreateSeasonForClubDate)(prisma, club, new Date());
    ...
    res.json({
        items: seasons.map((season) => (0, season_1.toSeasonResponse)(season)),
...
app.get('/clubs/me/coaches', authMiddleware, async (req, res) => {
    ...
    const pendingItems = (0, coach_invite_list_1.filterLatestCoachInvites)(invites, acceptedEmails);
    res.json([
...
app.post('/coaches/:id/invite', authMiddleware, async (req, res) => {
    if (!ensureDirection(req, res))
        return;
    if (!req.auth?.clubId)
        return res.status(404).json({ error: 'Club not found' });
...
app.put('/coaches/:id/teams', authMiddleware, async (req, res) => {
    if (!ensureDirection(req, res))
        return;
    if (!req.auth?.clubId)
        return res.status(404).json({ error: 'Club not found' });
    const schema = zod_1.z.preprocess((raw) => {
        if (!raw || typeof raw !== 'object' || Array.isArray(raw))
            return raw;
        const payload = raw;
        return {
        ...
        managedTeamIds: zod_1.z.array(zod_1.z.string().min(1)).default([]),
    }));
    const parsed = schema.safeParse(req.body);
    if (!parsed.success) {
        const firstIssue = parsed.error.issues[0];
    ...
    const requestedManagedTeamIds = (0, coach_management_1.normalizeCoachManagedTeamIds)(null, parsed.data.managedTeamIds);
    ...
    if (coach) {
        const updated = await prisma.user.update({
            where: { id: coach.id },
            data: {
                teamId: (0, coach_management_1.resolveCoachActiveTeamId)(coach.teamId, managedTeamIds) ?? nextActiveTeamId,
                managedTeamIds,
            },
            select: {
                id: true,
    ...
    const updatedInvite = await prisma.accountInvite.update({
        where: { id: invite.id },
        data: {
            teamId: (0, coach_management_1.resolveCoachActiveTeamId)(invite.teamId, managedTeamIds) ?? nextActiveTeamId,
            managedTeamIds,
        },
        select: {
            id: true,
...
app.delete('/coaches/:id', authMiddleware, async (req, res) => {
    if (!ensureDirection(req, res))
        return;
    if (!req.auth?.clubId)
        return res.status(404).json({ error: 'Club not found' });
...
app.put('/clubs/me', authMiddleware, async (req, res) => {
    ...
    const schema = zod_1.z.object({
        ...
        seasonConfig: season_1.clubSeasonConfigSchema.optional(),
    ...
    if (!currentClub)
        return res.status(404).json({ error: 'Club not found' });
    const nextSeasonConfig = parsed.data.seasonConfig ? (0, season_1.normalizeClubSeasonConfig)(parsed.data.seasonConfig) : null;
    const currentSeasonConfig = (0, season_1.normalizeClubSeasonConfig)(currentClub);
    ...
    if (nextSeasonConfig)
        Object.assign(data, (0, season_1.buildSeasonConfigPatch)(nextSeasonConfig));
    const updated = await prisma.club.update({
        where: { id: req.auth.clubId },
        data,
    });
    const currentSeason = await (0, season_1.resolveOrCreateSeasonForClubDate)(prisma, updated, new Date());
...
app.post('/accounts', authMiddleware, async (req, res) => {
    ...
    const filteredManagedTeamIds = requestedManagedTeamIds.length
        ...
        : [];
    const managedTeamIds = (0, coach_management_1.normalizeCoachManagedTeamIds)(selectedTeam.id, filteredManagedTeamIds);
    if (existing) {
...
const getPlayerByIdHandler = async (req, res) => {
    ...
    const parentInvites = parentLinkWhere
        ...
        : [];
    const parentContacts = (0, player_parent_contacts_1.summarizeParentContacts)(parentInvites);
    res.json({
        ...normalizedPlayer,
        parentContacts,
...
const updatePlayerRosterStatusHandler = async (req, res) => {
    if (!ensureStaff(req, res))
        return;
    const { id } = req.params;
    const existing = await playerFindFirstForUser(prisma, req.auth, { where: { id } });
    ...
    const [enrichedPlayer] = await enrichPlayersWithTeamAndClubContext([updated]);
    return res.json((0, player_payload_1.normalizePlayerForApi)(enrichedPlayer));
};
for (const route of player_route_aliases_1.playerDetailRouteAliases) {
...
app.post('/players/:id/invite', authMiddleware, async (req, res) => {
    ...
    const invitePhone = inviteRole === 'PARENT'
        ...
        : ((parsed.data.phone || '').trim() || player.phone || null);
...
<p>(Ces liens sont valables 60 jours)</p>`
    ...
    const accountInviteEmail = inviteRole === 'PARENT'
        ? (parsed.data.email || (0, player_parent_contacts_1.normalizeParentInviteEmail)(targetParentInvite?.email) || null)
        : inviteEmail;
    ...
    if (inviteRole !== 'PARENT') {
        try {
            (0, player_payload_1.assertPlayerAccountInvitePrerequisites)(player, { email: contactEmail, phone: accountInvitePhone });
        }
        catch (e) {
            if (e instanceof zod_1.z.ZodError) {
                return res.status(400).json({
        ...
        if (parsed.data.email)
            contactPatch.email = parsed.data.email.trim();
        if (parsed.data.phone)
            contactPatch.phone = parsed.data.phone.trim();
        if (Object.keys(contactPatch).length > 0) {
            ...
            if (contactPatch.email)
                player.email = contactPatch.email;
            if (contactPatch.phone)
                player.phone = contactPatch.phone;
        }
    }
    let invitationEmail = contactEmail;
    if (!invitationEmail && inviteRole === 'PARENT') {
        invitationEmail = `parent+${(0, nanoid_1.nanoid)(10)}@invite.izifoot.local`;
    }
    if (!invitationEmail) {
...
app.put('/trainings/:id', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (data.date) {
            data.seasonId = (await resolveSeasonForClubId(prisma, existing.clubId ?? req.auth?.clubId ?? null, data.date))?.id ?? null;
        }
    ...
    try {
        ...
        const rows = await prisma.$transaction(async (tx) => {
            ...
            const preservedInactiveItems = existingRows
                .filter((row) => !activePlayerIdSet.has(row.playerId))
                .map((row) => ({
...
app.put('/trainings/:id/roles', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const savedRows = await prisma.$transaction(async (tx) => {
            ...
            const preservedInactiveItems = existingRows
                .filter((row) => row.playerId && row.player?.is_active === false && !playerIds.includes(row.playerId))
                .map((row) => ({
...
app.get('/matchday', async (req, res, next) => {
    ...
    res.json({
        items: matchdays.map((matchday) => toEntityWithSeason(matchday)),
...
app.post('/matchday', authMiddleware, async (req, res) => {
    if (!ensureStaff(req, res))
        return;
    const parsed = matchday_metadata_1.matchdayCreateSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: parsed.error.flatten() });
    let team;
    try {
    ...
    try {
        ...
        if (e?.code === 'P2022')
            return res.status(503).json({ error: 'Matchday storage unavailable' });
    ...
    try {
        ...
        const matches = matchesRawWithContractKeys.map((m) => {
            ...
            return {
                ...
                season: season ? (0, season_1.toSeasonResponse)(season) : null,
                status,
                played: (0, match_status_1.derivePlayedFromStatus)(status),
...
app.get('/attendance', authMiddleware, async (req, res) => {
    ...
    if (seasonId) {
        ...
        const trainingIds = seasonTrainings.map((row) => row.id);
        const matchdayIds = seasonMatchdays.map((row) => row.id);
        const seasonScopedOr = [];
        if (!session_type || session_type === 'TRAINING') {
            seasonScopedOr.push({
                session_type: { in: ['TRAINING', 'TRAINING_ABSENT'] },
...
async function getMatchDetailForUser(db, scopeOrUserId, id) {
    ...
    return {
        ...
        season: match.season ? (0, season_1.toSeasonResponse)(match.season) : null,
        rotationGameKey: match.rotationGameKey ?? null,
        ...
        const updatedId = await prisma.$transaction(async (tx) => {
            ...
            if (payload.matchdayId !== undefined) {
                if (payload.matchdayId) {
                    ...
                    nextSeasonId = (await resolveSeasonForClubId(tx, existing.clubId ?? req.auth?.clubId ?? null, standaloneDate))?.id ?? null;
                }
            }
